...
const ProjectGlobe = () => {
    ...
    return (
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-full sm:w-[95vw] md:w-[90vw] h-[50vh] sm:h-[60vh] md:h-[70vh] lg:h-[80vh] flex justify-center items-end">
            {/* Floating astronaut */}
            <motion.img
                src={astronaut}
                className="absolute bottom-[85%] md:bottom-[65%] z-50 h-12 sm:h-16 md:h-20 lg:h-24"
                alt="astronaut"
            ...
            <AnimatePresence mode="wait">
                <motion.a
                    ...
                    key={project.id}
                    className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl text-glow text-white font-bebas-neue font-medium absolute bottom-[72%] md:bottom-[55%] z-50 tracking-wider text-center w-full px-4"
            ...
            <AnimatePresence mode="wait">
                {!isBoxOpen ? (
                    <motion.img
                        key="closed"
                        src={BoxClose}
                        className="absolute bottom-4 sm:bottom-8 md:bottom-12 lg:bottom-20 z-50 w-20 h-20 sm:w-28 sm:h-28 md:w-36 md:h-36 lg:w-40 lg:h-40 left-1/2 -translate-x-1/2 cursor-pointer"
                        onClick={() => toggleBox()}
                        alt="BoxClose"
                    ...
                    <motion.div
                        key="opened"
                        className="absolute bottom-4 sm:bottom-8 md:bottom-12 lg:bottom-20 left-1/2 -translate-x-1/2 z-50 w-[85%] sm:w-[55%] md:w-[40%] lg:w-[35%]"
                    ...
                    >
                        ...
                        />
                        <motion.div
                            className="p-2 sm:p-3 md:p-4 lg:p-5 absolute inset-0 flex flex-col items-center justify-center gap-1 sm:gap-2"
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            transition={{ delay: 0.4, duration: 0.5 }}
                        >
                            <motion.p
                                className="text-white/90 font-mono text-[0.65rem] sm:text-xs md:text-sm text-center leading-tight px-2"
                            ...
                            >
                                ...
                                {project.demoUrl && (
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-[0.6rem] sm:text-xs font-mono text-black bg-[#ceb7ff] hover:bg-white hover:shadow-[0_0_12px_rgba(206,183,255,0.6)] transition-all duration-300"
                                    >
                                        Demo
                                    </a>
                                )}
