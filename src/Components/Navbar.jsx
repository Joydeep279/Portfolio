...
const Navbar = ({ sections, currentIndex, onNavigate }) => {
  return (
    <motion.nav
      className="fixed top-0 left-0 right-0 z-[200] flex justify-center items-center py-4 px-6"
      style={{
    ...
    >
      {/* Logo */}
      <motion.a
        className="absolute left-4 sm:left-6 md:left-10 font-death-star text-[#CEB7FF] text-lg sm:text-xl md:text-2xl cursor-pointer font-outline text-glow select-none"
      ...
      <div className="flex items-center gap-2 sm:gap-4 md:gap-8">
        {sections.map((item, index) => (
          <motion.button
            key={item.id}
            className="relative px-3 py-1.5 sm:px-4 sm:py-2 font-bebas-neue text-sm sm:text-lg md:text-xl tracking-wider cursor-pointer border-none bg-transparent outline-none"
            style={{
              color:
                currentIndex === index
                  ? "#CEB7FF"
      ...
      {/* Contact CTA */}
      <motion.button
        className="absolute right-4 sm:right-6 md:right-10 px-4 py-1.5 sm:px-5 sm:py-2 font-bebas-neue text-sm sm:text-base md:text-lg tracking-wider rounded-full cursor-pointer border-none outline-none"
        style={{
          background:
